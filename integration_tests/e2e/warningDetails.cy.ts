context('Warning Details page', () => {
  ...
  it('when existing contact checkbox is checked; notes, requirement details and update button shown', () => {
    ...
    cy.get('#wholeSentence-0-2').should('be.checked')

    cy.get('#conditional-wholeSentence-0-2').should('exist').should('be.visible')
    cy.get('#conditional-wholeSentence-0-2').should('contain.text', 'Requirements')
    cy.get('#conditional-wholeSentence-0-2')
  ...
  it('when new contact checkbox is checked; notes, message and add button shown', () => {
    ...
    cy.get('#contact-1').should('exist')
    cy.get('#contact-1').should('not.be.checked')
    cy.get('#conditional-1').should('exist').should('not.be.visible')
    cy.get('#contact-1').click()
    cy.get('#wholeSentence-1-2').click()
    cy.get('#conditional-wholeSentence-1-2').should('exist').should('be.visible')
    cy.get('#conditional-wholeSentence-1-2').should('contain.text', 'Requirements')
    cy.get('button').contains('enforceablecontact_1').should('exist').should('be.visible')
  })
  ...
  it('failure notes twistie displayed and can be expanded', () => {
    ...
    cy.get('#wholeSentence-0-2').click()
  ...
  it('should retain contact information when contact is no longer returned by api', () => {
    cy.visit('/warning-details/0adf652a-45eb-4203-8104-7bb4e62be6ce')
    cy.url().should('include', '/warning-details')
    cy.get('.govuk-heading-l').should('exist').should('contain.text', 'Warning Details')
    cy.get('#contact-0').should('exist')
    cy.get('#contact-0').should('be.checked')
    cy.get('#wholeSentence-0-2').should('be.checked')
    cy.get('label[for="contact-0"]')
      .should('contain.text', '2/1/2025')
      .should('contain.text', 'Contact0Type')
      .should('contain.text', 'Contact0SubType')
    cy.get('#updatereq-button').should('exist').should('be.visible')
    cy.get('#updatereq-button')
  ...
  it('should show whole sentence radio buttons if a contact is selected', () => {
    cy.visit('/warning-details/1000000a-45eb-4203-8104-7bb98989845u')
    cy.url().should('include', '/warning-details')
    cy.get('.govuk-heading-l').should('exist').should('contain.text', 'Warning Details')
    cy.get('#contact-0').should('exist')
    cy.get('#contact-0').should('be.checked')
    cy.get('#wholeSentence-0-2').should('exist')
    cy.get('#wholeSentence-0').should('exist')
    cy.get('#wholeSentence-0-2').should('be.checked')
  })

  it('should fail validation if whole sentence is selected and no reason selected', () => {
    cy.visit('/warning-details/1000000a-45eb-4203-8104-7bb98989845u')
    cy.url().should('include', '/warning-details')
    cy.get('.govuk-heading-l').should('exist').should('contain.text', 'Warning Details')
    cy.get('#contact-0').should('exist')
    ...
    cy.get('#wholeSentence-0').should('exist')
    cy.get('#wholeSentence-0-2').should('be.checked')
    cy.get('#wholeSentence-0').click()
    cy.get('#continue-button').click()
    cy.contains('Please select a valid failure reason for each failure selected')
  })
})
